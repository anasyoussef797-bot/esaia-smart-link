...
export const QuickEditDestinationModal: React.FC<QuickEditDestinationModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Target URL updated instantly. Physical prints will now redirect to the new URL.', 'success');
      onClose();
    } catch (err) {
      showToast('Failed to update destination URL', 'error');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-neutral-800 flex items-center justify-between bg-neutral-950/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Quick Edit Target Destination</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="p-3 rounded-xl bg-emerald-950/20 border border-emerald-800/40 text-xs text-emerald-300 flex items-start gap-2">
            ...
            <div>
              <span className="font-semibold">Dynamic Zero-Reprint Binding</span>: All physical copies with shortcode{' '}
              <code className="font-mono font-bold text-white">/q/{qr.publicCode}</code> will immediately redirect to the new destination.
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="dynamic_url">Dynamic Smart Routing</option>
              <option value="menu">Digital Food &amp; Beverage Menu</option>
              <option value="vcard">Executive vCard Contact</option>
              <option value="page">Landing Page / Bio Card</option>
          ...
          <div>
            ...
            <input
              ...
              placeholder="https://yourbrand.com/new-campaign"
