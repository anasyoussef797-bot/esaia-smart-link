  ...
  const navSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'whitelabel', label: isRtl ? 'الهوية البيضاء' : 'White-Labeling', icon: <Sparkles className="w-4 h-4" />, path: '/admin/settings/white-label' },
        { id: 'export', label: isRtl ? 'تصدير البيانات' : 'Data Export', icon: <Download className="w-4 h-4" />, path: '/admin/settings/export' },
  ...
  return (
    <>
      ...
      <aside
        ...
        className={clsx(
          'fixed lg:sticky top-0 left-0 rtl:left-auto rtl:right-0 z-40 h-screen w-64 bg-[#0e1017] border-r rtl:border-r-0 rtl:border-l border-[#24293d] flex flex-col shrink-0 transition-transform duration-200 lg:translate-x-0',
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-16 border-b border-[#1c2030] shrink-0">
          <div className="flex items-center gap-2.5 cursor-pointer max-w-[200px]" onClick={() => onNavigate('/admin/overview')}>
            {currentOrg?.branding?.logoDarkUrl || currentOrg?.branding?.logoUrl ? (
              <div className="w-8 h-8 rounded-xl bg-[#141722] border border-[#24293d] flex items-center justify-center p-1 shrink-0 overflow-hidden">
                <img
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-[#1a1e2d] text-blue-400 border border-blue-900/40">
                ENTERPRISE
              </span>
            </div>
          </div>
                    ...
                    <button
                      ...
                      className={clsx(
                        ...
                        isActive
                          ...
                          : 'text-slate-400 hover:text-slate-200 hover:bg-[#141722] border border-transparent'
                      )}
                    >
                      <div className="flex items-center gap-2.5">
                        <span className={clsx(isActive ? 'text-blue-400' : 'text-slate-400')}>
                          {item.icon}
                        </span>
                        <span>{item.label}</span>
                      </div>
                      {item.badge && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-950 text-blue-400 font-mono border border-blue-800/40">
                          {item.badge}
                        </span>
                      )}
                    </button>
        ...
        {/* Footer Info */}
        <div className="p-3.5 border-t border-[#1c2030] bg-[#090a0f]/60 shrink-0">
